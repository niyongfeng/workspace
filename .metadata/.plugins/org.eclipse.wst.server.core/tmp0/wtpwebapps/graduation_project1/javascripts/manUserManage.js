...
function select(){
	...
	$.ajax({
		...
		success : function(data) {
			var json = eval('('+data+')');
			console.log(json);
			$("input[name='username']").val(json.name);
			$("select[name = 'status']").val(json.status);
			$("input[name='userpwd']").val(json.pwd);
			$("select[name = 'code']").val(json.code);
		}
	});
}
...
$(function() {
	...
	function InitTable(pageIndex) {
		...
		$
				.ajax({
					type : "get",
					url : 'ManUserManageServlet?pageIndex=' + pageIndex
