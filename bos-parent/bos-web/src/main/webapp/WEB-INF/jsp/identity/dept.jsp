		...
		<script type="text/javascript">
			$(function(){
				...
				$("#dept_datagrid").datagrid({
					...
					url : "${path}/identity/selectDept.jspx", // 数据表格需要请求的URL 数据格式: {} json格式
					columns : [[ // 列的相关信息(表头)
				...
				$("#addDept").click(function(){
					...
					$("<iframe style='width:100%;height:200px;' frameborder='0'></iframe>")
					  .attr("src", "${path}/identity/showAddDept.jspx") // 设置属性
					  .appendTo(divDialog);
				});
				...
				$("#updateDept").click(function(){
					...
					if (rows.length == 0){
						...
						/** 创建iframe */
						$("<iframe style='width:100%;height:200px;' frameborder='0'></iframe>")
						  .attr("src", "${path}/identity/showUpdateDept.jspx?dept.id=" + rows[0].id) // 设置属性
						  .appendTo(divDialog);
					}else{
						$.messager.alert("警告信息","修改部门时，只能选择一行！","warning");
					}
				});
					...
					if (rows.length == 0){
						...
						$.messager.confirm("确认信息", "您确定要删除吗？", function(r){
							if (r){
								...
								/** 异步请求删除部门 */
								$.post("${path}/identity/deleteDept.jspx",{ids : ids.join(",")},function(data, status){
									if (status == "success"){
			...
			<table>
				<tr>
					<shiro:hasPermission name="dept:add">
						<td>
							<a class="easyui-linkbutton" id="addDept" data-options="iconCls:'icon-add'">添加</a>
						</td>
					</shiro:hasPermission>
					
					<shiro:hasPermission name="dept:update">
						<td>
							<a class="easyui-linkbutton" id="updateDept" data-options="iconCls:'icon-edit'">修改</a>
						</td>
					</shiro:hasPermission>
					
					<shiro:hasPermission name="dept:delete">					
						<td>
							<a class="easyui-linkbutton" id="deleteDept" data-options="iconCls:'icon-remove'">删除</a>
