		...
		<script type="text/javascript">
			$(function(){
				...
				$("#job_datagrid").datagrid({
					...
					url : "${path}/identity/selectJob.jspx", // 数据表格需要请求的URL 数据格式: {} json格式
					columns : [[ // 列的相关信息(表头)
				...
				$("#addJob").click(function(){
					...
					$("<iframe style='width:100%;height:200px;' frameborder='0'></iframe>")
					  .attr("src", "${path}/identity/showAddJob.jspx") // 设置属性
					  .appendTo(divDialog);
				});
				...
				$("#updateJob").click(function(){
					...
					var rows = $("#job_datagrid").datagrid("getSelections");
					console.log(rows);
					if (rows.length == 0){
						...
						/** 创建iframe */
						$("<iframe style='width:100%;height:200px;' frameborder='0'></iframe>")
						  .attr("src", "${path}/identity/showUpdateJob.jspx?job.id=" + rows[0].id) // 设置属性
						  .appendTo(divDialog);
					}else{
						$.messager.alert("警告信息","修改职位时，只能选择一行！","warning");
					}
				});
					...
					if (rows.length == 0){
						...
						$.messager.confirm("确认信息", "您确定要删除吗？", function(r){
							if (r){
								...
								/** 异步请求删除职位 */
								$.post("${path}/identity/deleteJob.jspx",{ids : ids.join(",")},function(data, status){
									if (status == "success"){
			...
			<table>
				<tr>
					<shiro:hasPermission name="job:add">
						<td>
							<a class="easyui-linkbutton" id="addJob" data-options="iconCls:'icon-add'">添加</a>
						</td>
					</shiro:hasPermission>
					<shiro:hasPermission name="job:update">
						<td>
							<a class="easyui-linkbutton" id="updateJob" data-options="iconCls:'icon-edit'">修改</a>
						</td>
					</shiro:hasPermission>
					
					<shiro:hasPermission name="job:delete">
						<td>
							<a class="easyui-linkbutton" id="deleteJob" data-options="iconCls:'icon-remove'">删除</a>
						</td>
					</shiro:hasPermission>
