...
/**
 * Servlet implementation class teacherUpdateCompetition
 */
@WebServlet("/teacherUpdateCompetition")
public class teacherUpdateCompetition extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public teacherUpdateCompetition() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!MybatisSessionFactory.isValidDate(dateString)) {
			out.print("<script>alert('时间修改失败！');window.location.href='teacherCompetition.jsp';</script>");
		} else {
			cd.update(c);
			session.commit();
			out.print("<script>alert('更新成功！');window.location.href='teacherCompetition.jsp';</script>");
		}
	}
